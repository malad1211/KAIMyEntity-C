...
public class NativeFunc {
    ...
    private static final String gameDirectory = MCinstance.mcDataDir.getAbsolutePath();
    ...
    public static NativeFunc GetInst() {
        if (inst == null) {
            ...
            if(!inst.GetVersion().equals(libraryVersion)){
                KAIMyEntity.logger.warn("Incompatible Version dll. / loaded ver -> " + inst.GetVersion() + " / required ver -> "+ libraryVersion);
                KAIMyEntity.logger.warn("Please restart or download dll.");
                try{
                    Files.move(Paths.get(gameDirectory, "KAIMyEntitySaba.dll"),Paths.get(gameDirectory, "KAIMyEntitySaba.dll.old"));
                }catch(Exception e){
                    KAIMyEntity.logger.info(e);
                }
            }
        }
        return inst;
